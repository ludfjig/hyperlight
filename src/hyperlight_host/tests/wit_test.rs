...
use hyperlight_testing::{c_simple_guest_as_string, simple_guest_as_string, wit_guest_as_string};
...
fn sb() -> TestSandbox<Host, MultiUseSandbox> {
    sb_from_guest(wit_guest_as_string().unwrap())
}

fn sb_from_guest(path: String) -> TestSandbox<Host, MultiUseSandbox> {
...
mod wit_test {
    ...
    use crate::{
        GuestBinary, UninitializedSandbox, c_simple_guest_as_string, sb, sb_from_guest,
    ...
    fn restore_wit_snapshot_replaces_rust_and_c_guests() {
        ...
        let mut rust_target = sb_from_guest(simple_guest_as_string().unwrap());
        assert_eq!(
        ...
        let mut c_target = sb_from_guest(c_simple_guest_as_string().unwrap());
        assert_eq!(
    ...
    fn restore_chain_replaces_each_guest() {
        ...
        let mut target = sb_from_guest(c_simple_guest_as_string().unwrap());
