...
fn small_change_snapshot_has_expected_size() {
    ...
    let data_size = new_layer.blob().data().unwrap().len();
    ...
    assert_eq!(new_layer.blob().storage().mem_size(), expected_layer_size);
}
...
fn small_change_oci_layer_has_expected_file_size() {
    ...
    let data_size = memory.layers().last().unwrap().blob().data().unwrap().len();
    let expected_layer_size = data_size
        .checked_add(memory.page_table_len())
        .unwrap()
...
fn sparse_v2_address_span_above_flat_limit_round_trips() {
    ...
    .unwrap();
    let storage =
        ReadonlySharedMemory::from_bytes(&vec![0x5a; host_page_size], host_page_size).unwrap();
    let blob = Arc::new(
        SnapshotBlob::new(
            storage,
            Some(gpa_start),
    ...
    let active_page_table_layer = snapshot.memory.active_page_table_layer();
    let mut layers = snapshot.memory.layers().to_vec();
    layers.push(layer);
    let memory =
        Arc::new(SnapshotMemory::new(layers.into_boxed_slice(), active_page_table_layer).unwrap());
...
fn snapshot_layer_metadata_round_trips_exactly() {
    ...
    let metadata = |snapshot: &Snapshot| {
        snapshot
            ...
            .map(|layer| {
                let blob = layer.blob();
                (
                    blob.storage().mem_size(),
                    blob.data().map(|data| (data.gpa_start(), data.len())),
                    layer.live_data().to_vec(),
                    blob.page_tables().cloned(),
                )
            })
            .collect::<Vec<_>>()
    };
    let expected_layers = metadata(&snapshot);
    let expected_active_layer = snapshot.snapshot_memory().active_page_table_layer();
    ...
    assert_eq!(
        loaded.snapshot_memory().active_page_table_layer(),
...
fn page_table_only_layer_round_trips_from_disk() {
    ...
    let child = manager
        ...
        .unwrap();
    let active_layer =
        &child.snapshot_memory().layers()[child.snapshot_memory().active_page_table_layer()];
    assert!(active_layer.blob().data().is_none());
    assert!(active_layer.live_data().is_empty());
    ...
    let active_layer =
        &loaded.snapshot_memory().layers()[loaded.snapshot_memory().active_page_table_layer()];
    assert!(active_layer.blob().data().is_none());
    assert!(active_layer.live_data().is_empty());
...
fn snapshot_active_layer_without_page_tables_rejected() {
    ...
    assert_err_contains(err, "active snapshot layer has no page tables");
}
...
fn save_new_tag_into_loaded_layout_preserves_live_mapping() {
    ...
    let mappings_before: Vec<Vec<u8>> = loaded_a
        ...
        .layers()
        .iter()
        .map(|layer| layer.blob().storage().as_slice().to_vec())
        .collect();
    let blobs_dir = path.join("blobs").join("sha256");
