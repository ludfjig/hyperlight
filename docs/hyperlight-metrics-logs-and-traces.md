...
To inspect the trace file generated by the guest, use the `trace_dump` crate at `src/trace_dump`. It is kept out of the main workspace because its rendering dependencies (piet, cairo, pango) pull in a large native graphics stack. Pass its manifest explicitly:

```bash
cargo run --manifest-path src/trace_dump/Cargo.toml -- <path_to_guest_symbols> <trace_file_path> list_frames
```
...
```bash
cargo run --manifest-path src/trace_dump/Cargo.toml -- ./src/tests/rust_guests/bin/debug/simpleguest ./trace/<UUID>.trace list_frames
```
